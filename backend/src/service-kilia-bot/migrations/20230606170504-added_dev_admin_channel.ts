...
export const up: MigrationFun = async ({ context: query }) => {
  try {
  await query.addColumn(ConfigDBTableName, 'devAdminChannelId', ConfigDBTableAttributes.devAdminChannelId)
  } catch (_) {}
}

export const down: MigrationFun = async ({ context: query }) => {
  await query.removeColumn(ConfigDBTableName, 'devAdminChannelId')
}
